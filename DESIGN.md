...
scope_note: Current STDS brand frontend scope uses this file only as visual
  reference. Booking, reservation, write forms, CMS, realtime availability, and
  authenticated flows are out of scope unless a later issue explicitly adds
  them. Public brand content should come from Cloudflare Pages build-time
  fetches against the core backend public readonly endpoints.
