...
scope_note: This file is historical visual reference only. Current STDS brand
  frontend implementation planning must follow `design_handoff_yide_site/` for
  product/design/content-model direction. TinaCMS owns editorial content, and
  the core backend remains the source for public property availability only.
  Booking, reservation, write forms, realtime availability, and authenticated
  flows are out of scope unless a later issue explicitly adds them.
